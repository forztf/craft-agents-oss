...
export function SessionMetadataPanel({ sessionId, closeButton }: SessionMetadataPanelProps) {
  ...
  const { t } = useTranslation('components/right-sidebar/SessionMetadataPanel')
  ...
  if (!sessionId) {
    return (
      <div className="h-full flex flex-col">
        <PanelHeader title={t('Chat Info')} actions={closeButton} />
        <div className="flex-1 flex items-center justify-center text-muted-foreground p-4">
          <p className="text-sm text-center">{t('No session selected')}</p>
        </div>
      </div>
    )
  }
  ...
  if (!session) {
    return (
      <div className="h-full flex flex-col">
        <PanelHeader title={t('Chat Info')} actions={closeButton} />
        <div className="flex-1 flex items-center justify-center text-muted-foreground p-4">
          <p className="text-sm text-center">{t('Loading session...')}</p>
        </div>
      </div>
    )
  }
        ...
        <div>
          ...
          <div className="rounded-lg bg-foreground-2 has-[:focus]:bg-background shadow-minimal transition-colors">
            <Textarea
              ...
              placeholder={notesLoaded ? t('Add notes...') : t('Loading...')}
