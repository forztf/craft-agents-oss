...
export function RightSidebar({ panel, sessionId, closeButton }: RightSidebarProps) {
  const { t } = useTranslation('components/app-shell/RightSidebar')
  switch (panel.type) {
    ...
    case 'files':
      ...
      return (
        <div className="h-full flex items-center justify-center text-muted-foreground">
          <p className="text-sm">{t('Files panel - Coming soon')}</p>
        </div>
      )
    ...
    case 'history':
      ...
      return (
        <div className="h-full flex items-center justify-center text-muted-foreground">
          <p className="text-sm">{t('History panel - Coming soon')}</p>
        </div>
      )
