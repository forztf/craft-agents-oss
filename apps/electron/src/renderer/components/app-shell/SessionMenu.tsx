...
export function SessionMenu({
  ...
  const { t } = useTranslation('components/app-shell/SessionMenu')
  // Share handlers
  const handleShare = async () => {
    ...
    if (result?.success && result.url) {
      ...
      toast.error(t('Failed to share'), { description: result?.error || 'Unknown error' })
    }
  }
  ...
  const handleUpdateShare = async () => {
    ...
    if (result && 'success' in result && result.success) {
      ...
      toast.error(t('Failed to update share'), { description: errorMsg })
    }
  }
  ...
  const handleRevokeShare = async () => {
    ...
    if (result && 'success' in result && result.success) {
      ...
      const errorMsg = result && 'error' in result ? result.error : undefined
      toast.error(t('Failed to stop sharing'), { description: errorMsg })
    }
  }
  ...
  const handleRefreshTitle = async () => {
    ...
    if (result?.success) {
      toast.success(t('Title refreshed'), { description: result.title })
    } else {
      toast.error(t('Failed to refresh title'), { description: result?.error || 'Unknown error' })
    }
  }
    ...
    <>
      ...
      {!hasUnreadMessages && hasMessages && (
        <MenuItem onClick={onMarkUnread}>
          ...
          <span className="flex-1">{t('Mark as Unread')}</span>
        </MenuItem>
      )}
      ...
      <MenuItem onClick={handleRefreshTitle}>
        <RefreshCw className="h-3.5 w-3.5" />
        <span className="flex-1">{t('Regenerate Title')}</span>
      </MenuItem>
      ...
      <MenuItem onClick={onOpenInNewWindow}>
        ...
        <span className="flex-1">{t('Open in New Window')}</span>
      </MenuItem>

      {/* View in Finder */}
      <MenuItem onClick={handleShowInFinder}>
        <FolderOpen className="h-3.5 w-3.5" />
        <span className="flex-1">{t('View in Finder')}</span>
      </MenuItem>

      {/* Copy Path */}
      <MenuItem onClick={handleCopyPath}>
        <Copy className="h-3.5 w-3.5" />
        <span className="flex-1">{t('Copy Path')}</span>
      </MenuItem>
