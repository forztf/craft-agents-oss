...
export function TaskActionMenu({ task, sessionId, onKillTask, onInsertMessage, onShowTerminalOverlay, className }: TaskActionMenuProps) {
  ...
  const { t } = useTranslation('components/app-shell/TaskActionMenu')
  ...
  const handleViewOutput = async () => {
    if (!onShowTerminalOverlay) {
      toast.error(t('Terminal overlay not available'))
      return
    }
    ...
    try {
      ...
      onShowTerminalOverlay({
        ...
        output: output || t('No output available yet'),
        description: task.intent,
        toolType: 'bash', // Use 'bash' for both shell and agent tasks
      })
      setOpen(false)
    } catch (err) {
      toast.error(t('Failed to load task output'))
    }
  }
    ...
    <DropdownMenu open={open} onOpenChange={setOpen}>
      <DropdownMenuTrigger asChild>
        <button
          ...
          title={t('Click for task actions')}
        >
          {/* Spinner */}
          ...
          <span className="opacity-60">
            {task.type === 'agent' ? t('Task') : t('Shell')}
          </span>
