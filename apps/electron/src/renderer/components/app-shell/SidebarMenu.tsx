...
export function SidebarMenu({
  ...
  const { t } = useTranslation('components/app-shell/SidebarMenu')
  ...
  if ((type === 'allSessions' || type === 'status' || type === 'flagged') && onConfigureStatuses) {
    return (
      <MenuItem onClick={onConfigureStatuses}>
        ...
        <span className="flex-1">{t('Configure Statuses')}</span>
      </MenuItem>
    )
  }
      ...
      <>
        {onAddLabel && (
          <MenuItem onClick={() => onAddLabel(labelId)}>
            ...
            <span className="flex-1">{t('Add New Label')}</span>
          </MenuItem>
        )}
        {onConfigureLabels && (
          <MenuItem onClick={() => onConfigureLabels(labelId)}>
            <Settings2 className="h-3.5 w-3.5" />
            <span className="flex-1">{t('Edit Labels')}</span>
          </MenuItem>
        )}
        {labelId && onDeleteLabel && (
          <>
            ...
            <MenuItem onClick={() => onDeleteLabel(labelId)}>
              ...
              <span className="flex-1">{t('Delete Label')}</span>
            </MenuItem>
          </>
        )}
      </>
      ...
      <>
        {onConfigureViews && (
          <MenuItem onClick={onConfigureViews}>
            <Settings2 className="h-3.5 w-3.5" />
            <span className="flex-1">{t('Edit Views')}</span>
          </MenuItem>
        )}
        {viewId && onDeleteView && (
          <>
            ...
            <MenuItem onClick={() => onDeleteView(viewId)}>
              <Trash2 className="h-3.5 w-3.5" />
              <span className="flex-1">{t('Delete View')}</span>
            </MenuItem>
          </>
        )}
      </>
  ...
  if (type === 'sources') {
    ...
    const learnMoreLabel = sourceType === 'api'
      ? t('Learn More about APIs')
      : sourceType === 'mcp'
        ? t('Learn More about MCP')
        : sourceType === 'local'
          ? t('Learn More about Local Folders')
          : t('Learn More about Sources')
      ...
      <>
        {onAddSource && (
          <MenuItem onClick={onAddSource}>
            <Plus className="h-3.5 w-3.5" />
            <span className="flex-1">{t('Add Source')}</span>
          </MenuItem>
        )}
        <Separator />
  ...
  if (type === 'skills' && onAddSkill) {
    return (
      <MenuItem onClick={onAddSkill}>
        <Plus className="h-3.5 w-3.5" />
        <span className="flex-1">{t('Add Skill')}</span>
      </MenuItem>
    )
  }
