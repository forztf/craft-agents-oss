...
export default function PermissionsSettingsPage() {
  ...
  return (
    <div className="h-full flex flex-col">
      <PanelHeader title={t('Permissions')} actions={<HeaderMenu route={routes.view.settings('permissions')} helpFeature="permissions" />} />
          ...
          <div className="px-5 py-7 max-w-3xl mx-auto">
            <div className="space-y-8">
              {isLoading ? (
                ...
                <>
                  {/* About Section */}
                  <SettingsSection title={t('About Permissions')}>
                    <SettingsCard className="px-4 py-3.5">
                      <div className="text-sm text-muted-foreground leading-relaxed space-y-1.5">
                        <p>
                          {t('Permissions control how much autonomy your agent has. In {mode} mode, the agent can only read and research — perfect for understanding a problem before committing to changes. When you\'re ready, switch to {mode2} mode to let the agent implement the plan autonomously.', { mode: t('Explore'), mode2: t('Execute') })}
                        </p>
                        <p>
                          {t('A good workflow: start in Explore to let the agent investigate, review the proposed plan, then execute with confidence.')}
                        </p>
                        <p>
                          <button
                            type="button"
                  ...
                  <SettingsSection
                    ...
                    description={t('App-level patterns allowed in Explore mode. Commands not on this list are blocked.')}
                    action={
                    ...
                    <SettingsCard className="p-0">
                      {defaultPermissionsData.length > 0 ? (
                        <PermissionsDataTable
                          ...
                          maxHeight={350}
                          fullscreen
                          fullscreenTitle={t('Default Permissions')}
                        />
                      ) : (
                        <div className="p-8 text-center text-muted-foreground">
                          <p className="text-sm">{t('No default permissions found.')}</p>
                          <p className="text-xs mt-1 text-foreground/40">
                            {t('Default permissions should be at {path}', { path: '~/.craft-agent/permissions/default.json' })}
                          </p>
                        </div>
                      )}
                    </SettingsCard>
                  ...
                  <SettingsSection
                    title={t('Workspace Customizations')}
                    description={t('Workspace-level patterns that extend the app defaults above.')}
                    action={
                      (() => {
                    ...
                    <SettingsCard className="p-0">
                      {customPermissionsData.length > 0 ? (
                        <PermissionsDataTable
                          ...
                          maxHeight={350}
                          fullscreen
                          fullscreenTitle={t('Workspace Customizations')}
                        />
                      ) : (
                        <div className="p-8 text-center text-muted-foreground">
                          <p className="text-sm">{t('No custom permissions configured.')}</p>
                          <p className="text-xs mt-1 text-foreground/40">
                            {t('Create a {file} file in your workspace to add custom rules.', { file: 'permissions.json' })}
                          </p>
                        </div>
                      )}
                    </SettingsCard>
