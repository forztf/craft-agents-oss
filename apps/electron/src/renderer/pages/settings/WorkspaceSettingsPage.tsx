...
export default function WorkspaceSettingsPage() {
  const { t } = useTranslation('pages/settings/WorkspaceSettingsPage')
  ...
  const handleModeToggle = useCallback(
    async (mode: PermissionMode, checked: boolean) => {
      ...
      if (newModes.length < 2) {
        setModeCyclingError(t('At least 2 modes required'))
  ...
  if (!activeWorkspaceId) {
    return (
      <div className="h-full flex flex-col">
        <PanelHeader title={t('Workspace Settings')} actions={<HeaderMenu route={routes.view.settings('workspace')} helpFeature="workspaces" />} />
        <div className="flex-1 flex items-center justify-center">
          <p className="text-sm text-muted-foreground">{t('No workspace selected')}</p>
        </div>
      </div>
    )
  }
      ...
      <div className="flex-1 min-h-0 mask-fade-y">
        <ScrollArea className="h-full">
          ...
          <div className="space-y-8">
            {/* Workspace Info */}
            <SettingsSection title={t('Workspace Info')}>
              <SettingsCard>
                <SettingsRow
                  label={t('Name')}
            ...
            <SettingsSection title={t('Permissions')}>
              <SettingsCard>
                <SettingsMenuSelectRow
                  ...
                  description={t('Control what AI can do')}
            ...
            {/* Mode Cycling */}
            <SettingsSection
              title={t('Mode Cycling')}
              description={t('Select which modes to cycle through with Shift+Tab')}
            >
              <SettingsCard>
            ...
            <SettingsSection title={t('Advanced')}>
              <SettingsCard>
                <SettingsRow
                  ...
                  description={workingDirectory || t('Not set (uses session folder)')}
                  action={
                ...
                />
                <SettingsToggle
                  label={t('Local MCP Servers')}
                  description={t('Enable stdio subprocess servers')}
